 ...
 */
export function getBusinessDayDate(date: Date): string {
...
export function getBusinessDayStart(dateStr: string): Date {
  ...
  const dt = new Date(y, m - 1, d, BUSINESS_DAY_START_HOUR, 0, 0, 0);
  return dt;
}
...
/**
 * Agrupa una fecha en su cadena de día de negocio (alias de getBusinessDayDate).
 */
